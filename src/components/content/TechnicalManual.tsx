...
const FileItem = ({ name, description }: { name: string, description: string }) => (
    <div className="py-1">
        <div className="flex items-center gap-2">
            <File className="h-4 w-4 text-foreground/80" />
            <code>{name}</code>
    ...
    <ContentWrapper title={t('techTitle')}>
      ...
      <ContentSection title={t('techLocalTitle')} id="local-deployment">
        ...
        <ol className="list-decimal list-inside space-y-4">
          <li>
            <strong>{t('techLocalCloneTitle')}</strong>
            <p>{t('techLocalCloneDesc')}</p>
            <CodeBlock>git clone https://github.com/agl5702/backup-guia-mango</CodeBlock>
          </li>
          <li>
            <strong>{t('techLocalVenvTitle')}</strong>
            <p>{t('techLocalVenvDesc')}</p>
            <CodeBlock>{t('techLocalVenvActivate')}</CodeBlock>
          </li>
          <li>
            <strong>{t('techLocalDepsTitle')}</strong>
            <p>{t('techLocalDepsDesc')}</p>
            <CodeBlock>pip install -r requirements.txt</CodeBlock>
          </li>
          <li>
            <strong>{t('techLocalMigrateTitle')}</strong>
            <p>{t('techLocalMigrateDesc')}</p>
            <CodeBlock>python manage.py makemigrations users tipo_siembra mercado herramientas_agricultura_precision hectareas suelo analisis_suelo variedad_mango cultivo analisis_foliar informes tablas_estadisticas plagas</CodeBlock>
            <p className="mt-2">{t('techLocalApplyMigrateDesc')}</p>
            <CodeBlock>python manage.py migrate</CodeBlock>
          </li>
          <li>
            <strong>{t('techLocalLoadDataTitle')}</strong>
            <p>{t('techLocalLoadDataDesc')}</p>
            <CodeBlock>python manage.py loaddata user.json tipo_siembra.json herramientas.json fertilidad_suelo.json ph.json textura_suelo.json tipo_suelo.json variedad.json mercado.json hectareas.json</CodeBlock>
          </li>
          <li>
            <strong>{t('techLocalRunTitle')}</strong>
            <p>{t('techLocalRunDesc')}</p>
            <CodeBlock>python manage.py runserver</CodeBlock>
          </li>
        </ol>
        <p className="mt-4">{t('techLocalUrl')} <code>http://127.0.0.1:8000</code>.</p>
      </ContentSection>
