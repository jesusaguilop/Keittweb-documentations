...
const translations: Translations = {
  ...
  folderKeittwebDesc: { es: 'Contenedor del proyecto Django. Su nombre es el nombre del proyecto.', en: 'Django project container. Its name is the project name.' },
  folderInit: { es: 'Indica a Python que esto es un paquete.', en: 'Tells Python this is a package.' },
  folderAsgi: { es: 'Configuración para servidores asíncronos.', en: 'Configuration for asynchronous servers.' },
  ...
  folderWsgi: { es: 'Configuración para servidores síncronos.', en: 'Configuration for synchronous servers.' },
  folderApiDesc: { es: 'App de Django que contiene la lógica de la API RESTful.', en: 'Django app containing the RESTful API logic.' },
  folderModels: { es: 'Define los modelos de datos (tablas de la base de datos).', en: 'Defines data models (database tables).' },
  folderViews: { es: 'Contiene la lógica para manejar las peticiones y respuestas (endpoints).', en: 'Contains the logic to handle requests and responses (endpoints).' },
  folderSerializers: { es: 'Convierte los modelos de Django a JSON y viceversa.', en: 'Converts Django models to JSON and vice versa.' },
  folderApiUrls: { es: 'Enrutador de URLs específico para la app `api`.', en: 'URL router specific to the `api` app.' },
  folderManage: { es: 'Utilidad de línea de comandos para tareas administrativas.', en: 'Command-line utility for administrative tasks.' },
